<template>
  <div>
    ...
    >
      <template class="g-item" v-for="rowItem in rows">
        <div
          class="g-item"
          v-bind:class="{
            'g-snake': cellItem.IsBody,
            'g-prize': cellItem.IsPrize,
            'g-crashed': cellItem.IsBody && crashed,
          }"
...
export default {
  ...
  mounted() {
    ...
    var myElement = document.getElementById("game-main");
  ...
  methods: {
    init() {
      ...
      for (let y = 0; y < this.maxXY; y++) {
        ...
        for (let x = 0; x < this.maxXY; x++) {
          this.rows[y].push(new CellItem(x, y, i));
          i++;
        }
      }
    ...
    moveHead(x, y) {
      this.bodyParts.unshift(new BodyPart(x, y));
